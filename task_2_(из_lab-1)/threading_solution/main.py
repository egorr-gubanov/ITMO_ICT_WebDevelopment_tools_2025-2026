...
def parse_github_profile(github_url, skill_name, lock):
    try:
        headers = {
...
def worker(args):
    github_url, skill_name, lock = args
    return parse_github_profile(github_url, skill_name, lock)
...
def main():
    ...
    threads = []
    for github_url, skill_name in github_profiles:
        thread = threading.Thread(target=worker, args=((github_url, skill_name, lock),))
